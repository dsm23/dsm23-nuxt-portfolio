  ...
  const projects: Project[] = [
    {
      ...
      to: "https://github.com/dsm23/dsm23-nuxt-portfolio",
...
<template>
  <Section>
    ...
    <div
      class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-3"
    >
      <div
        v-for="{ title, to, description } in projects"
        :key="`project-${title}`"
      >
        <div :class="styles.imgContainer">
          <SvgsHelp
            class="absolute h-full w-full rounded-lg object-cover shadow-md"
          />
        </div>
        <div class="isolate -mt-16 px-4 print:mt-0">
          <div class="rounded-lg bg-white p-6 shadow-lg">
            ...
            <Anchor :href="to" class="mt-2 flex items-center gap-x-2">
              <SvgsNavRight class="h-6 w-6" />
